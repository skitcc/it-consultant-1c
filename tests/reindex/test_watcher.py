...
def test_owui_database_handler_notifies_on_webui_db() -> None:
    from watchdog.events import FileModifiedEvent

    calls: list[int] = []
    handler = OpenWebUIDatabaseHandler(lambda: calls.append(1))
    handler.on_any_event(FileModifiedEvent("/data/webui.db"))
    handler.on_any_event(FileModifiedEvent("/data/webui.db-wal"))
    handler.on_any_event(FileModifiedEvent("/data/webui.db-shm"))
    handler.on_any_event(FileModifiedEvent("/data/uploads/doc.pdf"))
    assert calls == [1, 1]
