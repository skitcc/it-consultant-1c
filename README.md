...
3. Письмо ставится в очередь. Первый обрабатывается сразу. Со 2-го места пользователь получает уведомление, сколько запросов перед ним и примерное ожидание (`QUEUE_WAIT_MINUTES`, по умолчанию 3 мин на запрос).
4. Загрузка треда, очистка тел. Служебные уведомления об очереди в историю модели не попадают.
5. Embedding вопроса → Qdrant `RAG_CANDIDATES` → rerank → `RAG_TOP_K` (соседи в том же разделе по `headings`) → фрагменты в `system_prompt`.
6. Non-stream вызов Ollama `/api/chat`: черновик с `think=medium`. Второй проход (`OLLAMA_VERIFIER_ENABLED`) по тем же чанкам с `think=high` по умолчанию выключен.
7. HTML-reply через EWS (таблицы, без Markdown и ссылок) + список использованных документов в конце.
8. Сбой, недоставленный ответ или несколько неотвеченных запросов пользователя в той же переписке — WARNING в лог и письмо на `ADMIN_EMAIL`.
9. При обрыве streaming — reconnect.
