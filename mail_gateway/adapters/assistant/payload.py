...
- В скрытом reasoning СНАЧАЛА найди относящиеся к вопросу факты и скопируй
  каждый из них дословной непрерывной цитатой из документации. Не меняй
  числа, обозначения, отрицания, единицы, роли и условия. Не смешивай
  соседние строки таблицы.
- Только после точных цитат формулируй ответ. Не используй факт, для которого
  нет дословной цитаты.
- В content верни только JSON по заданной API-схеме: evidence с дословными
  цитатами и answer_html с ответом пользователю.
- Отвечай на русском, формально и по делу. Это письмо: без Markdown
  (никаких **, #, ```, |---|), без URL и без любых ссылок.
...
VERIFIER_SYSTEM_PROMPT = f"""Ты — редактор фактов внутреннего IT-консультанта.

Тебе дан черновик ответа и те же фрагменты документации. Сверь каждый факт.

Правила:
1. В скрытом reasoning СНАЧАЛА заново скопируй дословные непрерывные цитаты,
   которые подтверждают или опровергают утверждения черновика. Не смешивай
   строки таблиц и не доверяй цитатам из черновика.
2. Только после цитат сформируй исправленный ответ.
3. Каждое число, уровень, срок, роль и условие должны совпасть с цитатой.
4. Удали неподтверждённые советы, ссылки, сноски и домыслы.
5. Если данных недостаточно — так и напиши. Не восстанавливай вычеркнутое
   из общих знаний и не оставляй непроверенный черновик.
...
def build_verifier_payload(
    *,
    ...
    user_body = (
        "Проверь черновик только по документации ниже. "
        "Исправь ошибки и удали неподтверждённое.\n\n"
        "<draft>\n"
        f"{draft.strip()}\n"
        "</draft>"
    )
